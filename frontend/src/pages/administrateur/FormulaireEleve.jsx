...
const schema = yup.object({
    matricule: yup.string().trim().required("Le numéro d'élève est obligatoire"),
...
export default function FormulaireEleve({ lectureSeule = false }) {
    ...
    async function enregistrer(donnees) {
        if (id) {
            await elevesApi.update(id, donnees);
        } else {
    ...
    return (
        <div>
            <header className="page-header">
                <h1>
                    {lectureSeule ? "Détails de l'élève" : id ? "Modifier l'élève" : "Ajouter un élève"}
                </h1>
            </header>
            <form className="card card-body form-card" onSubmit={handleSubmit(enregistrer)} noValidate >
                <div className="form-grid">
                    {[
                        ...
                        <div className="form-group" key={field[0]}>
                            ...
                            <input className="field" type={field[2] || "text"} disabled={lectureSeule} {...register(field[0])}/>
                    ...
                    <div className="form-group">
                        <label>Classe</label>
                        <select className="field" disabled={lectureSeule}
                    ...
                    <div className="form-group autocomplete">
                        ...
                        <input className="field" disabled={lectureSeule} value={rechercheParent}
                        ...
                        />
                        <input type="hidden" {...register("parentId", { valueAsNumber: true })} />
                        ...
                        {errors.parentId && ( <small className="error-text">{errors.parentId.message}</small>)}
                    </div>
                </div>
